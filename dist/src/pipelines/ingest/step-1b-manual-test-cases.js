...
function parseAssertionHint(expected) {
...
function normalizeCase(rawCase) {
    ...
    const stepExpected = rawCase.steps.flatMap((step) => typeof step === "string" ? [] : (step.expectedResults || []).map((line) => line.trim()));
