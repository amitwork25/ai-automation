function findCatalogTemplate(catalog, rule) {
    if (rule.assertFn) {
...
function findApprovedMapping(approved, rule) {
    if (!approved) {
        return undefined;
    }
...
function mapRule(rule, catalog, approved) {
...
export function step6RuleMapper(input) {
    ...
    const mappings = input.businessRules.rules.map((rule) => mapRule(rule, input.assertionCatalog, input.approvedRules));
