...
import { apiMethodBaseName, apiServiceClassName, modelInterfaceName, modelRelativePath, responseSchemaRelativePath, serviceRelativePath, serviceSpecRelativePath } from "./api-naming.js";
import { jsonSchemaDocument, jsonSchemaToInterface } from "./schema-to-typescript.js";
function coreImportPrefix(apiId, productId) {
    const depth = serviceRelativePath(apiId, productId).split("/").length - 1;
    return "../".repeat(depth);
}
function modelImportPrefix(apiId, productId) {
...
export function renderModelFile(api, schemas) {
    const responseName = modelInterfaceName(api, "response");
    const responseSchema = api.responseSchemaRef ? schemas[api.responseSchemaRef] : undefined;
    const chunks = [
    ...
    if (api.requestSchemaRef) {
        ...
        const requestSchema = schemas[api.requestSchemaRef];
        chunks.push(jsonSchemaToInterface(requestName, requestSchema));
    }
    return `${chunks.join("\n\n")}\n`;
}
export function renderResponseSchemaFile(api, schemas) {
    const responseSchema = api.responseSchemaRef ? schemas[api.responseSchemaRef] : undefined;
    return `${JSON.stringify(jsonSchemaDocument(`${api.method} ${api.path} success body`, responseSchema), null, 2)}\n`;
}
export function renderRequestSchemaFile(api, schemas) {
    const requestSchema = api.requestSchemaRef ? schemas[api.requestSchemaRef] : undefined;
    return `${JSON.stringify(jsonSchemaDocument(`${api.method} ${api.path} request body`, requestSchema), null, 2)}\n`;
}
export function renderServiceFile(api, productId, stepMap) {
...
export function renderServiceSpecFile(api, productId, stepMap) {
    const className = apiServiceClassName(api.apiId, stepMap);
    const methodBase = apiMethodBaseName(api.apiId, stepMap);
    ...
    const headersLine = isEqx && headersImportPath && headersBuilder
        ...
        : isEqx
            ...
            : headersImportPath && headersBuilder
                ? `import { ${headersBuilder} } from "${headersImportPath}";`
                : "";
    const authSetup = isEqx && !headersBuilder
