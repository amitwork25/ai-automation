...
import { assertFunctionName, buildPlanBodyLines, contextTypeName, personaFolder, renderJourneyFile, renderJourneySpecFile, renderTypesFile, runFunctionName } from "./render-integration-files.js";
function integrationBase(productId, persona) {
...
export async function step13JourneyCodegen(input) {
    ...
    for (const journey of input.journeyPlan.journeys) {
        try {
            ...
            const specPath = path.join("tests/integration", ...input.productId.split("/"), `${personaFolder(journey.persona)}-journey.generated.spec.ts`);
