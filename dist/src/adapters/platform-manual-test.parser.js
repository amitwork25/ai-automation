 ...
 */
export function parseInterleavedStepLines(stepsText) {
...
function isNullEndpointToken(value) {
    const token = value.trim().toLowerCase();
    return token === "" || token === "null" || token === "none" || token === "-";
}
function parseEndpointPart(part) {
 ...
 */
export function parseStepEndpoints(raw) {
...
export function applyStepEndpoints(steps, endpoints) {
...
function inferJourneyTags(name, testId) {
    const lower = name.toLowerCase();
...
export function isPlatformManualResponse(raw) {
    ...
    const hasPlatformFields = ("test_id" in first || "testId" in first) &&
...
export function parsePlatformManualCase(raw, index) {
...
export function parsePlatformManualResponse(raw) {
    return raw.response
        .filter(isRecord)
