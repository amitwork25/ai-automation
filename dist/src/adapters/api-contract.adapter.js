...
function isPostmanCollection(raw) {
...
function buildApiIdFromPath(path, fallbackName) {
    const segments = path
        .split("/")
...
function parseContractRow(raw, index) {
    ...
    const apiId = asString(raw.apiId || raw.api_id || raw.id || raw.operationId || raw.operation_id, buildApiIdFromPath(path, asString(raw.name || raw.title, `api_${index + 1}`)));
    return {
        apiId,
        method,
        path,
        ...
        responseFields: Array.isArray(raw.responseFields)
            ...
            : [],
        samples: isRecord(raw.samples) ? raw.samples : undefined
    };
}
...
function adaptPrebuiltContracts(raw, productId) {
    ...
    if (Array.isArray(payload)) {
        ...
        if (isRecord(payload.schemaIndex) && isRecord(payload.schemaIndex.schemas)) {
            schemas = payload.schemaIndex.schemas;
        }
    }
    if (apis.length === 0) {
 ...
 */
export function adaptApiContracts(raw, productId, openApiSpec) {
    const payload = unwrapApiPayload(raw);
    if (isRecord(payload) && isPostmanCollection(payload)) {
        return {
            kind: "postman",
            postmanCollection: payload,
    ...
    throw new Error("Unsupported API contract format. Provide Postman collection, OpenAPI spec, or apis[] list.");
}
export function materializeApiArtifacts(source, productId) {
