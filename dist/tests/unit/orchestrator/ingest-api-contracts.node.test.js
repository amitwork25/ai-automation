...
function baseState(openApiSpec) {
    return {
        runId: "run-001",
        productId: "bbps/ccbp",
...
describe("ingestApiContractsNode", () => {
    it("accepts openApiSpec as the API contract source", async () => {
        ...
        const apiContracts = await artifactStore.readJson("run-001", "02-api-contracts.json");
    ...
    it("reports all supported API input keys when missing", async () => {
        ...
        tempDirs.push(testDir);
        const artifactStore = new FileArtifactStore(path.join(testDir, "runs"));
        await expect(ingestApiContractsNode(baseState(), artifactStore)).rejects.toThrow("apiContracts, postmanCollection, or openApiSpec is required");
    });
});
