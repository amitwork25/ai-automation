...
describe("P0 ingest + linker pipeline", () => {
    it("produces stable 04 journey spec with mappingRate >= 0.95", async () => {
        const manualInput = await readJson("manual-test-cases.json");
        const postmanInput = await readJson("postman-collection.json");
        const openApiInput = await readJson("openapi.json");
