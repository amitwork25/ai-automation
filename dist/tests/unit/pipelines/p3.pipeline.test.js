...
describe("P3 journey planner + integration codegen", () => {
    it("builds 07 journey plan with resolved step calls", async () => {
        ...
        expect(plan.journeys[0]?.steps.some((step) => step.checkpoint?.includes("assertCcbpNewUserJourneyContracts"))).toBe(true);
    ...
    it("generates journey.generated.ts and spec in greenfield temp repo", async () => {
        ...
        const repoIndex = {
            ...
            repoRoot,
            services: [],
            steps: (await readFixture("03-repo-index.json")).steps,
            journeys: [],
            assertions: [],
            specs: [],
            folderLayout: {},
