...
import type {
  ...
  ApprovedKnowledgeIndexArtifact,
  ApprovedRulesConfig,
...
export async function step18Approve(input: {
  ...
  projectRoot?: string;
}): Promise<{ approvedRules: ApprovedRulesConfig; result: Omit<ApprovalResultArtifact, "knowledgeIndexPath" | "rulesApprovedPath"> }> {
  ...
  const approvedEntries: ApprovedRulesConfig["rules"] = [];
  ...
  const approvedRules: ApprovedRulesConfig = {
...
export async function persistApprovedKnowledge(input: {
  ...
  approvedRules: ApprovedRulesConfig;
  projectRoot?: string;
}): Promise<{ rulesApprovedPath: string; knowledgeIndexPath: string }> {
  const projectRoot = input.projectRoot ?? process.cwd();
  ...
  const knowledgeIndexPath = path.join(
    ...
    "agent-knowledge",
    input.productId,
    "03a-approved-knowledge-index.json"
  );
