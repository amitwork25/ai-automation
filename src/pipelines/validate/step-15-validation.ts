...
async function runOptionalCommand(check: ValidationCheck, command: string, args: string[], cwd: string): Promise<ValidationCheck> {
  try {
...
function collectGeneratedFiles(
  reports: Array<{ generated?: Array<{ files: string[] }> }>
): string[] {
...
export async function step15Validation(input: {
  ...
  apiCodegenReport: ApiCodegenReportArtifact;
  serviceTestReport: ServiceTestCodegenReportArtifact;
  assertionReport: AssertionCompileReportArtifact;
  journeyReport: JourneyCodegenReportArtifact;
  runExternalChecks?: boolean;
}): Promise<ValidationReportArtifact> {
  const checks: ValidationCheck[] = [];
  const gapsRemaining: string[] = [];
  ...
  const validationStatus: ValidationReportArtifact["validationStatus"] = failed ? "fail" : gapsRemaining.length > 0 ? "warn" : "pass";
