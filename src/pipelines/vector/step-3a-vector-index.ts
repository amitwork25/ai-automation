...
import { MemoryVectorStore, type VectorDocument } from "./memory-vector-store.js";

async function readSnippet(repoRoot: string, relativePath: string, maxChars = 1200): Promise<string> {
  try {
...
export async function step3aVectorIndex(input: {
...
}): Promise<{ report: VectorIndexReportArtifact; store: MemoryVectorStore }> {
  ...
  const collectionCounts = documents.reduce<Record<string, number>>((counts, document) => {
