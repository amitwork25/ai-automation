...
async function serviceFileExists(root: string, relativePath: string): Promise<boolean> {
  try {
...
export async function step10ServiceTestCodegen(input: {
...
}): Promise<ServiceTestCodegenReportArtifact> {
  const generated: ServiceTestCodegenReportArtifact["generated"] = [];
  const skipped: ServiceTestCodegenReportArtifact["skipped"] = [];
  const errors: ServiceTestCodegenReportArtifact["errors"] = [];
  ...
  for (const api of targetApis) {
    try {
      ...
      const hasService =
        input.repoIndex.services.some((entry) => entry.className === className) ||
