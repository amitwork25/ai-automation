...
import type { ICustomAssertLlmClient } from "../../ports/ICustomAssertLlmClient.js";
...
export async function step12CustomAsserts(input: {
  ...
  persona: string;
  retrievalContext?: VectorRetrievalContextArtifact;
  llmClient?: ICustomAssertLlmClient;
  useLlm?: boolean;
}): Promise<CustomAssertCompileReportArtifact> {
  ...
  const generated: CustomAssertCompileReportArtifact["generated"] = [];
  const skipped: CustomAssertCompileReportArtifact["skipped"] = [];
  const errors: CustomAssertCompileReportArtifact["errors"] = [];
...
`;
  ...
  const outputPath = path.join(
    ...
    personaFolder(input.persona),
    "custom-asserts.generated.ts"
  );
