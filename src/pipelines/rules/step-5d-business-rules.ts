...
interface ParsedEnglishRule {
  layer: BusinessRule["layer"];
  path?: string;
  op?: BusinessRule["op"];
  expected?: unknown;
  assertFn?: string;
...
const ENGLISH_RULE_PATTERNS: Array<{
  pattern: RegExp;
  build: (text: string) => ParsedEnglishRule | ParsedEnglishRule[] | null;
}> = [
  {
...
function lookupJourneyContext(
  journeySpec: JourneySpecArtifact,
  caseId: string,
  manualStepIndex: number
): { journeyId?: string; apiIds?: string[] } {
  for (const journey of journeySpec.journeys) {
    if (!journey.sourceCaseIds.includes(caseId)) {
      continue;
    }
    const mapping = journey.stepMappings.find(
      (entry) => entry.caseId === caseId && entry.manualStepIndex === manualStepIndex
    );
    if (mapping) {
...
function parseEnglishExpected(text: string): ParsedEnglishRule[] {
...
function pushRule(
  rules: BusinessRule[],
  unmappedRules: BusinessRulesArtifact["unmappedRules"],
  input: {
    caseId: string;
    text: string;
    manualStepIndex?: number;
    ...
    parsed?: ParsedEnglishRule;
    hint?: { path: string; op: BusinessRule["op"]; value: unknown };
  }
): void {
...
export function step5dBusinessRules(input: {
  ...
  const unmappedRules: BusinessRulesArtifact["unmappedRules"] = [];
  let totalExpected = 0;
  let parsedCount = 0;
