...
import type { IRuleGapLlmClient } from "../../ports/IRuleGapLlmClient.js";

function reportProvider(): LlmReportProvider {
  return resolveLlmProvider();
}

function applyResolution(rule: BusinessRule, resolution: RuleGapResolution): BusinessRule {
...
export async function step5gRuleGaps(input: {
  ...
  llmClient: IRuleGapLlmClient;
}): Promise<{ businessRules: BusinessRulesArtifact; report: RuleGapsReportArtifact }> {
  ...
  const parsedCount =
    input.businessRules.parseMeta.parsedCount +
