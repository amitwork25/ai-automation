import type { DependencyGraphArtifact, JourneySpecArtifact } from "../../contracts/pipeline.js";
import type { LinkerFallbackMapping } from "../../ports/ILinkerFallbackLlmClient.js";
...
export function applyLinkerLlmFallback(input: {
  ...
  const accepted = input.mappings.filter(
    (entry) => entry.apiIds.length > 0 && entry.confidence >= MIN_CONFIDENCE
  );
  ...
  const acceptedKeys = new Set(
    accepted.map((entry) => `${entry.caseId}:${entry.manualStepIndex}`)
  );

  const unmapped = input.journeySpec.unmapped.filter(
    (entry) => !acceptedKeys.has(`${entry.caseId}:${entry.manualStepIndex}`)
  );
  ...
  const journeys = input.journeySpec.journeys.map((journey) => {
    ...
    const apiSequence = topologicalSortSubset(
      [...journey.apiSequence, ...newApiIds],
