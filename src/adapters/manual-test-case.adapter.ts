import type { ManualStepApiRef, ManualTestCaseInput } from "../contracts/pipeline.js";
import {
  isPlatformManualResponse,
  parsePlatformManualResponse
} from "./platform-manual-test.parser.js";
...
function unwrapManualPayload(raw: unknown): unknown {
  if (!isRecord(raw)) {
    return raw;
  }
...
function parseApiRef(raw: unknown): ManualStepApiRef | undefined {
  if (!isRecord(raw)) {
    return undefined;
  }
  ...
  const pathHint = asString(
    raw.pathHint || raw.path || raw.url || raw.endpoint || raw.route || raw.path_hint
  );
...
function parseApiRefs(raw: unknown): ManualStepApiRef[] {
  if (Array.isArray(raw)) {
    return raw.map((entry) => parseApiRef(entry)).filter((entry): entry is ManualStepApiRef => !!entry);
  }
  const single = parseApiRef(raw);
  return single ? [single] : [];
}

function parseFlexibleStep(raw: unknown): ManualTestCaseInput["steps"][number] {
  ...
  const action = asString(
    raw.action || raw.step || raw.testStep || raw.test_step || raw.description || raw.text
  );
  const expectedResults = asStringArray(
    raw.expectedResults || raw.expectedResult || raw.expected || raw.result || raw.results
  );
...
function parseCase(raw: UnknownRecord, index: number): ManualTestCaseInput {
  const stepsRaw =
    raw.steps || raw.testSteps || raw.test_steps || raw.actions || raw.flow || raw.scenario;
  let steps: ManualTestCaseInput["steps"] = [];
...
function isManualCaseShape(raw: UnknownRecord): boolean {
  return (
 ...
 */
export function adaptManualTestCases(raw: unknown, productId: string): ManualTestCaseInput[] {
  ...
  if (Array.isArray(payload)) {
    ...
    if (isRecord(payload[0])) {
      ...
      const hasStepExpectedPair = maybePairs.every(
        (entry) =>
          asString(entry.action || entry.step || entry.testStep) &&
  ...
  if (isRecord(payload)) {
    ...
    if (testSteps.length > 0) {
      const steps =
        expectedResults.length === testSteps.length
  ...
  throw new Error(
    "Unsupported manual test case format. Provide cases/steps/testSteps with actions and optional apiRef."
  );
}
