...
import type { IRuleGapLlmClient, RuleGapLlmInput } from "../../ports/IRuleGapLlmClient.js";
...
const UI_ONLY_PATTERN =
  /lands on|entry is available|can start the journey|screen loads|open the app|dashboard|navigation|without already having/i;
const ASSERT_FN_PATTERN = /\b(assert[A-Z][A-Za-z0-9_]*)\b/;

function hitsForRule(
  input: RuleGapLlmInput,
  rule: { caseId: string; text: string; manualStepIndex?: number }
...
export class HeuristicRuleGapLlmClient implements IRuleGapLlmClient {
  async resolveRuleGaps(input: RuleGapLlmInput): Promise<RuleGapResolution[]> {
...
export class GeminiRuleGapLlmClient implements IRuleGapLlmClient {
  constructor(
  ...
  async resolveRuleGaps(input: RuleGapLlmInput): Promise<RuleGapResolution[]> {
    ...
    const parsed = data as { resolutions?: RuleGapResolution[] };

    return parsed.resolutions ?? [];
  }
}

export class OpenAiRuleGapLlmClient implements IRuleGapLlmClient {
  constructor(
    private readonly apiKey: string,
  ...
  async resolveRuleGaps(input: RuleGapLlmInput): Promise<RuleGapResolution[]> {
    ...
    const parsed = JSON.parse(content) as { resolutions?: RuleGapResolution[] };
    return parsed.resolutions ?? [];
  }
}

export function createRuleGapLlmClient(): IRuleGapLlmClient {
