...
import type {
  ILinkerFallbackLlmClient,
  LinkerFallbackLlmInput,
  LinkerFallbackMapping
} from "../../ports/ILinkerFallbackLlmClient.js";
import { callGeminiJsonChatWithFallback } from "./gemini-json.client.js";
import { loadLlmSystemPrompt } from "./load-llm-prompt.js";

export class GeminiLinkerFallbackLlmClient implements ILinkerFallbackLlmClient {
  constructor(
    private readonly apiKey: string,
    private readonly model: string
  ) {}

  async mapUnmappedSteps(input: LinkerFallbackLlmInput): Promise<LinkerFallbackMapping[]> {
    ...
    const parsed = data as { mappings?: LinkerFallbackMapping[] };
...
export class OpenAiLinkerFallbackLlmClient implements ILinkerFallbackLlmClient {
  constructor(
    private readonly apiKey: string,
    private readonly model: string
  ) {}
  ...
  async mapUnmappedSteps(input: LinkerFallbackLlmInput): Promise<LinkerFallbackMapping[]> {
    ...
    const parsed = JSON.parse(content) as { mappings?: LinkerFallbackMapping[] };
    return {
      mappings: parsed.mappings ?? [],
...
export function createLinkerFallbackLlmClient(): ILinkerFallbackLlmClient | null {
