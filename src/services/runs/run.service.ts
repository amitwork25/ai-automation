...
import type { ApiAliasesConfig } from "../../contracts/pipeline.js";
...
export interface RunDetails {
  ...
  approvalStatus?: ApprovalManifestArtifact["approvalStatus"];
}

export interface ApproveRunResult {
  runId: string;
  productId: string;
  approval: ApprovalResultArtifact;
}

export interface IRunService {
  createRun(input: CreateRunRequest): Promise<RunResult>;
  getRun(runId: string): Promise<RunDetails>;
  approveRun(runId: string): Promise<ApproveRunResult>;
}

export class RunService implements IRunService {
  constructor(
    private readonly frameworkIntelligence: IFrameworkIntelligenceService,
    private readonly artifactStore: IArtifactStore,
    private readonly projectRoot = process.cwd()
  ) {}

  async createRun(input: CreateRunRequest): Promise<RunResult> {
  ...
  async getRun(runId: string): Promise<RunDetails> {
    ...
    const meta = await this.artifactStore.readJson<{
      runId: string;
      productId: string;
      repoRoot: string;
      codegenRoot?: string;
    }>(runId, "00-run-meta.json");

    const artifacts = Object.fromEntries(
      artifactNames.map((name) => [name, path.join("runs", runId, name)])
    );

    let approvalStatus: ApprovalManifestArtifact["approvalStatus"] | undefined;
    if (artifactNames.includes("18-approval-manifest.json")) {
      const manifest = await this.artifactStore.readJson<ApprovalManifestArtifact>(
        runId,
  ...
  async approveRun(runId: string): Promise<ApproveRunResult> {
    const artifactNames = await this.artifactStore.listArtifacts(runId);
    ...
    const meta = await this.artifactStore.readJson<{ productId: string }>(runId, "00-run-meta.json");
    const [businessRules, mappedRules] = await Promise.all([
      this.artifactStore.readJson<BusinessRulesArtifact>(runId, "05-business-rules.json"),
      this.artifactStore.readJson<BusinessRulesMappedArtifact>(runId, "06-business-rules-mapped.json")
    ]);
    ...
    const approval: ApprovalResultArtifact = {
      ...result,
      rulesApprovedPath,
      knowledgeIndexPath
    };
