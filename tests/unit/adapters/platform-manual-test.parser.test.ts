...
import {
  parseInterleavedStepLines,
  parsePlatformManualResponse,
  parseStepEndpoints
} from "../../../src/adapters/platform-manual-test.parser.js";
...
describe("platform-manual-test.parser", () => {
  ...
  it("parses full endpoint path and METHOD path in endpoint key", () => {
    const endpoints = parseStepEndpoints(
  ...
  it("parses arrow-separated single-line steps", () => {
    const steps = parseInterleavedStepLines(
      "Open app → enter mobile → enter wrong OTP → submit."
    );
