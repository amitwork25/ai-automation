...
describe("P1 repo index + rules pipeline", () => {
  it("indexes CCBP framework repo with steps, journeys, and assertions", async () => {
    ...
    expect(repoIndex.assertions.some((entry) => entry.name === "assertCcbpNewUserJourneyContracts")).toBe(
      true
    );
  ...
  it("parses TC39 expected results and maps rules to templates", async () => {
    ...
    expect(
      mappedRules.mappings.some(
        (entry) => entry.templateId === "assert.ccbpNewUserJourneyContracts" || entry.templateId === "expect.length.zero"
      )
    ).toBe(true);
  });
});
