...
import type { BusinessRulesArtifact } from "../../../src/contracts/pipeline.js";
...
async function readFixture<T>(name: string): Promise<T> {
  ...
  return JSON.parse(await readFile(path.join(p3FixturesDir, name), "utf8")) as T;
}
...
describe("P5 vector + rule gaps", () => {
  ...
  it("resolves unmapped rules via heuristic 5g using retrieval context", async () => {
    const businessRules: BusinessRulesArtifact = {
  ...
  it("skips 5g when no unmapped rules remain after 5d", async () => {
    const businessRules = await readFixture<BusinessRulesArtifact>("05-business-rules.json");
