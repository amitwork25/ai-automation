...
import type { ApiContract, RepoIndexArtifact } from "../../../src/contracts/pipeline.js";
...
const emptyRepoIndex = (repoRoot: string): RepoIndexArtifact => ({
...
describe("P2 API + service test codegen", () => {
  it("generates model, schema, and service files for a missing API", async () => {
    ...
    expect(report.generated[0]?.files).toEqual(
    ...
    const serviceSource = await readFile(
      path.join(repoRoot, "src/services/eqx/bbps/ccbp/CcbpHomepageRemindersService.ts"),
      "utf8"
    );
  ...
  it("generates a smoke service spec for newly generated APIs", async () => {
    ...
    const specSource = await readFile(
      path.join(repoRoot, "tests/service/bbps/ccbp/homepage-reminders/homepage-reminders.service.spec.ts"),
      "utf8"
    );
