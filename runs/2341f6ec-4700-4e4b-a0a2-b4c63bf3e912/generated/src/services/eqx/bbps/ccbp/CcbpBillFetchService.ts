...
import type { CcbpBillFetchResponse } from "../../../../../models/eqx/bbps/ccbp/bill-fetch.models";
...
export class CcbpBillFetchService {
  ...
  private endpointPath(): string {
    return "/bbps/v1/ccbp/bill-fetch";
  }

  async billFetchRaw(headers: Record<string, string>): Promise<ApiCallResult<CcbpBillFetchResponse>> {
  ...
  async billFetch(headers: Record<string, string>): Promise<CcbpBillFetchResponse> {
    const result = await this.billFetchRaw(headers);
